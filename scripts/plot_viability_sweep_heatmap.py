...
def main():
    ...
    df = pd.read_csv('results/viability_sweep.csv')
    def req_shots(o):
        o = max(min(float(o), 0.999999), 1e-12)
        return -4.605/np.log(1.0 - o)
    df['required_shots_99'] = df['overlap'].apply(req_shots)
    # Pivot by N (rows) and p_or_layers (columns)
    pivot = df.pivot_table(index='N', columns='p_or_layers', values='required_shots_99', aggfunc='mean')
    ...
    out = 'results/plots/viability_sweep_heatmap.png'
