...
from .energy_qaoa_simulator import get_expectation as _get_expectation, get_overlap as _get_overlap, get_probabilities as _get_probabilities
...
class QAOAFastSimulatorPythonBase(QAOAFastSimulatorBase):
    ...
    def get_probabilities(self, result: np.ndarray, **kwargs) -> np.ndarray:
        return _get_probabilities(result, **kwargs)

    def get_expectation(self, result: np.ndarray, costs: np.ndarray | None = None, optimization_type="min", **kwargs) -> float:
        if costs is None:
            costs = self._hc_diag
        probabilities = self.get_probabilities(result, **kwargs)
        return _get_expectation(probabilities, costs, optimization_type, **kwargs)
    ...
    def get_overlap(
        ...
        return _get_overlap(probabilities, costs, indices, optimization_type, **kwargs)
